...
def splitESB(inputESB, output_workspace):
    ...
    EMSOutput = join(working_dir, "ESB_EMS_BOUNDARY") # Changed to OK Fields
    FireOutput = join(working_dir, "ESB_FIRE_BOUNDARY") # Changed to OK Fields
    LawOutput = join(working_dir, "ESB_LAW_BOUNDARY") # Changed to OK Fields
    ...
    stats_eff_date = join("in_memory", "EffectDate")
    Statistics_analysis(lyrESB, stats_eff_date, [["EffectDate", "COUNT"]], "EffectDate")

    eff_date = ""
    high_count = 0
    with SearchCursor(stats_eff_date, ("EffectDate", "COUNT_EFF_DATE")) as rows:
        for row in rows:
            if row[1] > high_count:
                high_count = row[1]
    ...
    for key in esbworkDict:
        ...
        Dissolve_management(layer, output, [esb_obj.DiscrpAgID, esb_obj.UNIQUEID, esb_obj.STATE, esb_obj.AGENCYID,
        ...
        fieldsDict = {"RevDate": ["DATE"], "EffectDate": ["DATE"], "ExpireDate": ["DATE"], "RevEditor": ["TEXT", 50], "SUBMIT": ["TEXT", 1, "SUBMIT"], "Comment": ["TEXT", 255]} # Changed to OK Fields
        ...
        for f in fieldsDict:
            ...
            if len(parameters) == 1:
                ...
                if f == "RevDate": # Changed to OK Fields
                    #set last update to now
                    CalculateField_management(output, f, 'datetime.datetime.now()', 'PYTHON_9.3')
                elif f == "EffectDate": # Changed to OK Fields
            ...
            if len(parameters) > 1:
                ...
                if f == "RevEditor": # Changed to OK Fields
    ...
    if Exists(EMSOutput) and Exists(FireOutput) and Exists(LawOutput):
        userMessage("ESBs split. NOTE: the EffectDate was filled in with the MOST COMMON EffectDate from the ESB layer. Please check individual data pieces for accuracy.")
    else:
