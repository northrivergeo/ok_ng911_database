            ...
            while start_int < field_count:
                if row[start_int] is not None:
                    if row[start_int] not in ("", " "):
                        label = label + "|" + unicode(row[start_int])
...
def main():
    ...
    if a > 0:
        filename = "ROAD_CENTERLINE" # Changed to OK Fields
        userMessage(unicode(a) + " records were in road labels but not address point road labels")
    ...
    #loop through unmatching records if necessary and report issues
    if b > 0:
        filename = "ADDRESS_POINT" # Changed to OK Fields
        userMessage(unicode(b) + " records were in address point road labels and not in road labels")
